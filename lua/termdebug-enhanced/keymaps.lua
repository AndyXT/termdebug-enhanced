...
function M.setup_keymaps(keymaps)
  ...
  if keymaps.toggle_breakpoint then
    ...
    if not valid then
      table.insert(errors, "Invalid toggle_breakpoint keymap '" .. keymaps.toggle_breakpoint .. "': " .. validation_error)
    else
  ...
  if keymaps.evaluate_visual then
    ...
    if not valid then
      table.insert(errors, "Invalid evaluate_visual keymap '" .. keymaps.evaluate_visual .. "': " .. validation_error)
    else
  ...
  if #errors > 0 then
    vim.notify("Keymap setup completed with errors (" .. success_count .. "/" .. total_count .. " successful):\n" .. 
               table.concat(errors, "\n"), vim.log.levels.WARN)
  else
    vim.notify("All keymaps set up successfully (" .. success_count .. "/" .. total_count .. ")", vim.log.levels.INFO)
  end
...
function M.cleanup_keymaps()
  ...
  if #errors > 0 then
    vim.notify("Keymap cleanup completed with errors (" .. success_count .. "/" .. total_count .. " successful):\n" .. 
               table.concat(errors, "\n"), vim.log.levels.WARN)
