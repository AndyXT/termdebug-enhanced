...
---@field switch_format function Switch memory display format
---@field set_format function Set specific memory display format
local M = {}
...
local function create_memory_window(content, opts, is_error)
	...
	if not is_error then
		...
		local help_text = {
			...
			"  h: hex | d: decimal | b: binary | 6: base64 | f: cycle format",
			"───────────────────────────────────────────────────────────────",
			"",
		}
...
local current_memory = {
	...
	size = 256,
	variable = nil,
	format = "hex", -- Current display format
}
...
---Format memory response based on current format setting
---@param response table GDB memory response lines
---@param format string Format type (hex, decimal, binary, base64)
---@param address string Memory address
---@param size number Number of bytes
---@param bytes_per_line number Bytes per display line
---@return table Formatted content lines
local function format_memory_response(response, format, address, size, bytes_per_line)
	local formatted = {
	...
	if format == "base64" then
		...
		-- Convert to base64 and format in chunks
		local base64_str = bytes_to_base64(all_bytes)
		local chars_per_line = 64
		for i = 1, #base64_str, chars_per_line do
			local chunk = base64_str:sub(i, i + chars_per_line - 1)
	...
	-- For hex, decimal, binary - format line by line with ASCII
		...
		if line and line ~= "" and not line:match("^%(gdb%)") then
			...
			if addr_match then
				...
				if data_part then
					...
					-- Extract bytes and format according to display format
					for hex_byte in data_part:gmatch("0x(%x%x)") do
						...
						if byte_val then
							...
							if format == "hex" then
								table.insert(formatted_values, string.format("0x%02x", byte_val))
							elseif format == "decimal" then
								table.insert(formatted_values, string.format("%3d", byte_val))
							elseif format == "binary" then
								local binary = ""
								for bit = 7, 0, -1 do
									local bit_val = math.floor(byte_val / (2^bit)) % 2
									binary = binary .. tostring(bit_val)
								end
								table.insert(formatted_values, binary)
							end
						end
					end
					...
					local ascii = ""
					for _, byte_val in ipairs(bytes) do
					...
					-- Format the line based on display format
					local values_str = table.concat(formatted_values, " ")
					local formatted_line
					
					if format == "binary" then
						-- Binary format needs more space
						formatted_line = string.format("%-12s %s", addr_match .. ":", values_str)
						if #ascii > 0 then
							formatted_line = formatted_line .. "  |" .. ascii .. "|"
						end
					else
						-- Hex and decimal formats
						formatted_line = string.format("%-18s %-48s |%s|", 
							addr_match .. ":", values_str, ascii)
					end
...
function M.show_memory(address, size)
	...
	local config = get_config()
	local format = current_memory.format or config.format or "hex"
	...
	-- Send memory examination command (always get hex bytes for consistent parsing)
	...
	utils.async_gdb_response(cmd, function(response, error)
		...
		if response and #response > 0 then
			...
			local formatted = format_memory_response(response, format, address, size, bytes_per_line)

			-- Add navigation and format hints
			table.insert(formatted, "")
			table.insert(formatted, string.rep("─", 70))
			table.insert(formatted, "Format: h(hex) d(decimal) b(binary) 6(base64) | +/- (navigate) | r(refresh) | e(edit)")
...
local function create_popup_window(content, opts, is_error)
	...
	if not is_error then
		...
		keymaps["h"] = function() M.set_format("hex") end
		keymaps["d"] = function() M.set_format("decimal") end
		keymaps["b"] = function() M.set_format("binary") end
		keymaps["6"] = function() M.set_format("base64") end
		keymaps["f"] = function() M.switch_format() end
...
function M._show_memory_popup_internal(address, size, variable, type_info)
	...
	utils.async_gdb_response(cmd, function(response, error)
		...
		if response and #response > 0 then
			...
			local format = current_memory.format or "hex"
			local formatted = {}
			if variable then
				...
				table.insert(formatted, string.format("🏠 Address: %s (%d bytes, %s)", address, size, format))
			else
				table.insert(formatted, string.format("🏠 Memory at %s (%d bytes, %s)", address, size, format))
			end
			table.insert(formatted, string.rep("─", 60))

			-- Use new formatting function (but make it more compact for popup)
			local memory_lines = format_memory_response(response, format, address, size, 16)
			-- Skip the header lines from format_memory_response and add the formatted content
			for i = 3, #memory_lines do  -- Skip "✓ Memory at..." and "───" lines
				table.insert(formatted, memory_lines[i])
			end

			-- Add help footer
			table.insert(formatted, "")
			table.insert(formatted, string.rep("─", 60))
			table.insert(formatted, "💾 s: save | S: buffer | c: compare | h/d/b/6: format | f: cycle | q: close")
...
---Switch to next memory display format
---@return nil
function M.switch_format()
	if not current_memory.address then
		vim.notify("No memory view active to change format", vim.log.levels.WARN)
		return
	end

	local formats = {"hex", "decimal", "binary", "base64"}
	local current_idx = 1
	...
	-- Switch to next format (wrap around)
	local next_idx = (current_idx % #formats) + 1
	current_memory.format = formats[next_idx]
	
	-- Refresh display with new format
	M.show_memory(current_memory.address, current_memory.size)
	vim.notify(string.format("Memory format: %s", current_memory.format), vim.log.levels.INFO)
end

---Set specific memory display format
---@param format string Format to set (hex, decimal, binary, base64)
---@return nil
function M.set_format(format)
	...
	if not is_valid then
		vim.notify("Invalid format: " .. format .. ". Valid formats: hex, decimal, binary, base64", vim.log.levels.ERROR)
		return
	end
	...
	current_memory.format = format
	M.show_memory(current_memory.address, current_memory.size)
	vim.notify(string.format("Memory format: %s", format), vim.log.levels.INFO)
end
