...
local format_handlers = {
	hex = function(bytes)
		return table.concat(vim.tbl_map(function(b) return string.format("0x%02x", b) end, bytes), " ")
	end,

	decimal = function(bytes)
		return table.concat(vim.tbl_map(function(b) return string.format("%3d", b) end, bytes), " ")
	end,

	binary = function(bytes)
		return table.concat(vim.tbl_map(function(b)
			local binary = ""
			for bit = 7, 0, -1 do
			...
			if addr_match then
				...
				if data_part then
					...
					local formatted_line = string.format("%-18s %-48s |%s|", 
						addr_match .. ":", formatted_values, ascii)
...
function setup_window_keymaps(buf, window_type, is_error)
	...
	for key, action in pairs(keymaps) do
		pcall(vim.api.nvim_buf_set_keymap, buf, "n", key, "", {
			callback = function() handle_keymap_action(action, window_type) end,
			noremap = true,
			silent = true,
		})
	end
...
function handle_keymap_action(action, window_type)
	local actions = {
		...
		refresh = function() M.refresh_memory() end,
		edit = function() M.edit_memory_interactive() end,
		save_register = function() M.save_memory_to_register() end,
		save_buffer = function() M.save_memory_to_buffer() end,
		compare = function() M.compare_memory_with_register() end,
		format_hex = function() M.set_format("hex") end,
		format_decimal = function() M.set_format("decimal") end,
		format_binary = function() M.set_format("binary") end,
		format_base64 = function() M.set_format("base64") end,
		cycle_format = function() M.switch_format() end,
		nav_forward = function() M.navigate_memory(16) end,
		nav_backward = function() M.navigate_memory(-16) end,
	}
...
local function edit_memory_bytes(address, bytes_str)
	...
	if #byte_list == 0 then return end
		...
		utils.async_gdb_response(string.format("set *%s = %s", addr, byte), function(_, error)
			...
			if completed == total then
				...
				if current_memory.address then
					vim.defer_fn(function() M.refresh_memory() end, 200)
				end
			end
		end)
	end
...
function M.show_memory_popup(address, size)
	...
	return with_validation({
		validators.gdb_available,
		function() return address and validators.valid_address(address) or true end,
	}, function()
		if not address then
...
function M.set_format(format)
	return with_validation({
		function() return validators.valid_format(format) end,
...
function M.edit_memory_at_cursor()
	...
	return with_validation({
		validators.gdb_available,
		function() return validators.valid_address(word) end,
	}, function()
		if word:match("^0x%x+") or word:match("^%d+") then
...
function M.edit_memory_interactive()
	return with_validation({validators.has_active_memory, validators.gdb_available}, function()
		...
		if not ok_offset then return end
		...
		if not ok_value or value == "" then return end
...
function M.save_memory_to_buffer(buffer_name)
	return with_validation({validators.has_active_memory}, function()
		...
		memory_ops.get_data(current_memory.address, current_memory.size, function(response, error)
			...
			local formatted = format_memory_response(response, current_memory.format, 
				current_memory.address, current_memory.size)
...
function M.compare_memory_with_register(register)
	return with_validation({validators.has_active_memory}, function()
		...
		memory_ops.get_data(current_memory.address, current_memory.size, function(response, error)
			...
			local current_lines = format_memory_response(response, current_memory.format,
				current_memory.address, current_memory.size)
